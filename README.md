...
5. В GitHub откройте **Settings → Secrets and variables → Actions → Repository secrets** и добавьте ровно эти secrets:
   ...
   - `supabase_db_url` — строка подключения к базе Supabase для применения `supabase-schema.sql` во время deploy (только server-side; она не записывается в browser artifact).
