...
The workflow validates that both secrets are present, writes `supabase-config.js` only inside the deployment artifact, and then publishes the site. Never place a Supabase `service_role` key in this repository or in any browser-delivered file.
