...
function chartMarkup(rows, emptyText) { if (!rows.length) return `<p>${emptyText}</p>`; const max = Math.max(...rows.map((row) => row.value), 1); return `<div class="chart" aria-label="${emptyText}">${rows.map((row) => `<div class="chart-row"><span class="chart-label">${row.label}</span><div class="chart-track"><div class="chart-bar" style="width: ${Math.max((row.value / max) * 100, 4)}%"></div></div><strong>${row.display}</strong></div>`).join('')}</div>`; }
...
function averageLikesChartMarkup(state) { const counts = dailyCounts(state.surveys); const rows = Object.entries(counts).sort((a, b) => a[0].localeCompare(b[0])).map(([day, count]) => { const value = state.users.length ? count / state.users.length : 0; return { label: day, value, display: averageDecimal([value]) }; }); return chartMarkup(rows, 'Пока недостаточно данных для графика среднего числа понравившихся.'); }
...
function storageErrorMessage(error) {
  ...
  if (error?.code === 'PGRST205' || error?.message?.includes('schema cache') || error?.message?.includes('sociopairs_users')) {
    return `PostgREST не видит таблицы sociopairs в проекте, к которому подключён сайт. Если SQL уже выполнен, проверьте, что repository secrets supabase_url и supabase_anonpublic указывают на тот же Supabase-проект, сайт был redeploy после изменения secrets, а в Supabase → Project Settings → API → Exposed schemas включена схема public. ${setupHint}`;
  }
