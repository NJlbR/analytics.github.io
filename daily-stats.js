...
renderProfile = function renderProfileWithDailyStats(state, user) {
  originalRenderProfile(state, user);
  if (!user) return;
...
renderAdmin = function renderAdminWithDailyStats(state, user) {
  originalRenderAdmin(state, user);
  if (!user?.isAdmin) return;

  const adminCard = document.querySelector('#adminSection .card');
  const table = adminCard.querySelector('.table-wrap');
  ...
  `;
  adminCard.insertBefore(summary, table);
};
